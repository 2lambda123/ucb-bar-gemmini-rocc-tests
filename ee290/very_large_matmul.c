...
void full_matmul(elem_t A[DIM_I][DIM_K], elem_t B[DIM_K][DIM_J],
                 int64_t C_full[DIM_I][DIM_J]) {
...
void full_matshift(int64_t full[DIM_I][DIM_J], elem_t out[DIM_I][DIM_J],
                   int shift) {
  for (size_t r = 0; r < DIM_I; r++)
    for (size_t c = 0; c < DIM_J; c++) {
      ...
      int64_t elem = shifted > elem_t_max
                         ? elem_t_max
