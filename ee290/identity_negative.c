...
int main() {
  ...
  // printf("Move \"Identity\" matrix from main memory into Gemmini's
  // scratchpad\n");
  gemmini_mvin(Identity, Identity_sp_addr);

  // printf("Multiply \"In\" matrix with \"Identity\" matrix with a bias of
  // 0\n");
  ...
  if (!is_equal(Correct, Out)) {
    printf(
        "Output matrix is not negative one multiplied by the input matrix!\n");
    printf("\"In\" matrix:\n");
    printMatrix(In);
    printf("\"Out\" matrix:\n");
    printMatrix(Out);
